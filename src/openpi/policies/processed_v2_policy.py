...
def make_processed_v2_example() -> dict:
...
class ProcessedV2Inputs(transforms.DataTransformFn):
    """
    ...
    # actual action dimension in your dataset
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # NOTE: passing zero state since we don't have proprioceptive info saved
        state_dim = self.dataset_action_dim - (self.dataset_action_dim // 7)  # remove gripper from action for state
        state = transforms.pad_to_dim(np.zeros((state_dim,), dtype=np.float32), self.action_dim)
...
@dataclasses.dataclass(frozen=True)
class ProcessedV2Outputs(transforms.DataTransformFn):
    """
    ...
    def __call__(self, data: dict) -> dict:
        ...
        return {"actions": np.asarray(data["actions"][:, :self.dataset_action_dim])}
